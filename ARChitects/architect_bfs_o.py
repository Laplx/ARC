...
def _bfs_generate(
    ...
    *,
    ...
    queue: deque[tuple[torch.Tensor, float, list[int]]] = deque([(start_ids, 0.0, [])])
    ...
    with torch.no_grad():
        while queue and len(results) < max_candidates and nodes < max_nodes:
            input_ids, score, generated = queue.popleft()
            nodes += 1
            ...
            for tid, lp in options[:top_k]:
                ...
                queue.append((torch.cat([input_ids, next_ids], dim=1), new_score, new_generated))
            if nodes % 100 == 0:
                print(
                    f"[architect_bfs_o] BFS progress: nodes={nodes}, results={len(results)}, queue={len(queue)}"
                )
    ...
    with torch.no_grad():
        for step, tid in enumerate(target_ids, 1):
            ...
            if step <= 3 or step == len(target_ids):
                # 打印前几步与最后一步，便于观察概率
                print(
                    f"[architect_bfs_o] logprob step {step}/{len(target_ids)} "
...
class ArchitectSolver:
    ...
    def solve(self, task, *, context: Mapping[str, object] | None = None):
        print("[architect_bfs_o] start solve")
        # 1) 预生成：仅对原任务做一次 BFS，得到候选网格。
        prompt_orig = self._codec.serialize_task(task, test_index=0)
        print("[architect_bfs_o] prompt sample (orig):")
        print(prompt_orig[:300] + ("..." if len(prompt_orig) > 300 else ""))
        print("[architect_bfs_o] running BFS on original task")
        raw_candidates = _bfs_generate(
            self._model,
            self._tokenizer,
            prompt_orig,
        ...
        print(f"[architect_bfs_o] BFS done, raw candidates: {len(raw_candidates)}")
        if raw_candidates:
            for i, (txt, lp) in enumerate(raw_candidates[:3], 1):
                print(f"[architect_bfs_o] raw cand {i}: logprob={lp:.2f}")
                print(txt[:200])
        ...
        for text, logprob in raw_candidates:
            ...
            if grid is None:
                print("[architect_bfs_o] deserialize failed, text head:", text[:120])
            parsed_candidates.append((grid, logprob))

        print(f"[architect_bfs_o] parsed candidates: {len(parsed_candidates)}")
        if not parsed_candidates:
            # 如果首次 BFS 没有解析出候选，尝试放宽条件重试几次
            max_retries = 1
            for retry in range(1, max_retries + 1):
                print(f"[architect_bfs_o] no candidates, retry {retry}/{max_retries}")
                raw_candidates = _bfs_generate(
                    self._model,
                    self._tokenizer,
                    prompt_orig,
                ...
                for text, logprob in raw_candidates:
                    ...
                    if grid is None:
                        print(
                            "[architect_bfs_o] retry deserialize failed, text head:",
                            text[:120],
                        )
                        continue
                    parsed_candidates.append((grid, logprob))
                print(
                    f"[architect_bfs_o] retry {retry} parsed candidates: {len(parsed_candidates)}"
                )
        ...
        print(f"[architect_bfs_o] transforms to score: {len(transforms)}")
        ...
        if len(parsed_candidates) == 1:
            ...
            print("[architect_bfs_o] single candidate, skip scoring")
            candidates_out.append({"grid": grid, "score": lp0, "meta": {"single": True}})
            return candidates_out

        for idx, (grid, _) in enumerate(parsed_candidates, 1):
            print(f"[architect_bfs_o] scoring candidate {idx}/{len(parsed_candidates)}")
            logprobs: list[float] = []
            for t_idx, record in enumerate(transforms, 1):
                ...
                if t_idx % 4 == 0 or t_idx == len(transforms):
                    print(
                        f"[architect_bfs_o] candidate {idx} scored {t_idx}/{len(transforms)} transforms"
                    )

            mean_logprob = sum(logprobs) / len(logprobs) if logprobs else float("-inf")
            print(
                f"[architect_bfs_o] candidate {idx} mean_logprob={mean_logprob:.2f} "
    ...
    *,
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            print("[architect_bfs_o] model moved to", target_device)
        else:
            print("[architect_bfs_o] cuda not available, stay on CPU")
    except Exception as exc:  # 防止因模型分布式加载报错
            print(f"[architect_bfs_o] warn: failed to move model to cuda: {exc}")
