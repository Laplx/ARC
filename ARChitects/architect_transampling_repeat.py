...
LOG_PATH = Path("outputs/architect_trans.log")
...
class NVARCArchitect:
    ...
    def transform_gen(self, task: dict) -> list[dict]:
        colors = _collect_colors(task)
        color_perms = _color_permutations(colors)
        transforms: list[dict] = []
        for geom_name, (geom_fn, inv_geom) in _TRANSFORMS.items():
            for perm_name, perm_map in color_perms[:2]:  # 生成阶段仅用前两种颜色变换，减少搜索空间
                inv_map = _invert_map(perm_map)
...
def _grid_key(grid: list[list[int]] | None) -> str:
    if grid is None:
        return "<none>"
...
def _beam_generate(
    ...
    *,
    ...
    """
    Sampling-first candidate generator.
    Generates max_candidates samples (or slightly more until enough unique grids).
    Deduplicates by parsed grid.
    """

    input_ids_base = tokenizer(prompt_text, return_tensors="pt").input_ids.to(device)
    attention_mask_base = torch.ones_like(input_ids_base, device=device)
    ...
    for t_idx, record in enumerate(transforms, 1):
        ...
        prompt_t = codec.serialize_task(transformed_task, test_index=0)
        
        prompt_ids = tokenizer(prompt_t, return_tensors="pt").input_ids.to(device)
        ...
        new_text_ids = generated_ids[prompt_len:]

        # decode only new tokens
        decoded = tokenizer.decode(new_text_ids, skip_special_tokens=False)

        grid = codec.deserialize_grid(decoded)
        if grid is None:
            print("[architect_beam] deserialize failed, skip candidate")
            continue
        transformed_grid = inverse(grid)
        key = _grid_key(transformed_grid)
        if key in seen:
            continue
        seen.add(key)
        print(f"[architect_beam] rollout {rollouts}/{max_rollouts} transform {t_idx}/{len(transforms)}",
              f"grid key: {key[:60]} candidates so far: {len(candidates)}",
              )
        ...
        candidates.append(
            {
                ...
                "text": decoded,
                # 你如果有算 rollout logprob 的逻辑，可以加在这里
                # "score": rollout_result.get("score", None),
            }
        )

        if len(candidates) >= max_candidates or rollouts >= max_rollouts:
            break
...
class ArchitectSolver:
    ...
    def solve(self, task, *, context: Mapping[str, object] | None = None):
        ...
        transforms_gen = self._architect.transform_gen(task)
        transforms = self._architect.transform(task)
        print(f"[architect_beam] transforms to generate: {len(transforms_gen)}",
              f"[architect_beam] transforms to score: {len(transforms)}")
        ...
        print(f"[architect_beam] Beam done, candidates: {len(raw_candidates)}")
        if raw_candidates:
            for cand_idx, cand in enumerate(raw_candidates, 1):
                print(f"[architect_beam] candidate {cand_idx} text head: {cand['grid'][:120]}")
        ...
        if len(raw_candidates) == 1:
            ...
            candidates_out.append({"grid": cand["grid"], "meta": {"single": True}})
            return candidates_out

        for idx, cand in enumerate(raw_candidates, 1):
            grid = cand["grid"]
            print(f"[architect_beam] scoring candidate {idx}/{len(raw_candidates)}")
            logprobs: list[float] = []
            for t_idx, record in enumerate(transforms, 1):
                transformed_task = record["task"]
