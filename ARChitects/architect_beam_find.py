...
def _beam_generate(
    ...
    *,
    ...
    """
    混合策略：默认 DFS，遇到高熵且低置信时、且 beam 次数未耗尽，再切换小宽度 beam。
    每条路径的 beam 使用次数受 beam_num 限制。
    """
    if not allowed_ids:
        return []
    ...
    # stack: (input_ids, score, generated, beam_used)
    stack: list[tuple[torch.Tensor, float, list[int], int]] = [(encoded["input_ids"], 0.0, [], 0)]
    ...
    # 阈值：gap 与熵，控制何时启用 beam
    base_gap_thr = 0.10  # 概率差阈值
    base_entropy_thr = 1.0  # 较高熵才触发
    base_pmax_thr = 0.8  # 最高概率过低时更可能触发
    gap_thr = base_gap_thr * relax_factor
    entropy_thr = base_entropy_thr / relax_factor
    pmax_thr = base_pmax_thr * relax_factor

    with torch.no_grad():
        while stack and len(results) < max_candidates and nodes < max_nodes:
            input_ids, score, generated, beam_used = stack.pop()
            nodes += 1
            ...
            # 计算集中度
            probs = [math.exp(lp) for _, lp in options]
            if len(probs) >= 2:
                gap = probs[0] - probs[1]
            else:
                gap = 1.0
            ...
            need_beam = (
                beam_used < beam_num
                and len(options) > 1
                and (gap < gap_thr or entropy > entropy_thr)
                and probs[0] < pmax_thr
            )
            if need_beam:
                print(
                    f"[architect_beam] trigger beam @len={len(generated)} "
                    f"(used={beam_used}/{beam_num}) gap={gap:.3f} entropy={entropy:.3f} "
                    f"pmax={probs[0]:.3f}"
                )

            children: list[tuple[torch.Tensor, float, list[int], int]] = []
            width = beam_width if need_beam else top_k
            for tid, lp in options[:width]:
                ...
                children.append(
                    (torch.cat([input_ids, next_ids], dim=1), new_score, new_generated, beam_used + (1 if need_beam else 0))
                )

            # DFS/LIFO：高概率的放栈底，先探索低概率
            for child in children:
                stack.append(child)

            if nodes % 100 == 0:
                print(
                    f"[architect_beam] progress: nodes={nodes}, results={len(results)}, stack={len(stack)}, beam_used={beam_used}"
                )
...
class ArchitectSolver:
    ...
    def solve(self, task, *, context: Mapping[str, object] | None = None):
        ...
        if not parsed_candidates:
            # 如果首次 Beam 没有解析出候选，尝试放宽条件重试几次
            max_retries = 1
            retry = 0
            while not parsed_candidates:
                retry += 1
                print(f"[architect_beam] no candidates, retry {retry}/{max_retries}")
                raw_candidates = _beam_generate(
                    ...
                    max_steps=self._max_steps,
                    max_candidates=self._max_candidates,
                    beam_width=self._beam_width * (retry + 1), # self._beam_width,
                    beam_num=self._beam_num * (retry + 1), # self._beam_num,
                    top_k=min(self._top_k + retry * 2, len(self._allowed_ids)), # len(self._allowed_ids),
                    max_nodes=self._max_nodes * (retry + 1), # self._max_nodes,
                    min_prob=max(self._min_prob / (retry + 1) ** 2, 0.0), # max(self._min_prob / (retry + 1), 0.0),
                    relax_factor=1.0 + retry * 0.5, # 1.0 + retry,
                )
                parsed_candidates = []
                for text, logprob in raw_candidates:
                    grid = self._codec.deserialize_grid(text)
                    if grid is None:
                        print(
                            "[architect_beam] retry deserialize failed, text head:",
                            text[:120],
                        )
                        continue
                    parsed_candidates.append((grid, logprob))
                print(
                    f"[architect_beam] retry {retry} parsed candidates: {len(parsed_candidates)}"
                )
        ...
        print(f"[architect_beam] transforms to score: {len(transforms)}")
        ...
        if len(parsed_candidates) == 1:
            ...
            print("[architect_beam] single candidate, skip scoring")
