...
LOG_PATH = Path("outputs/architect_bb.log")
...
def _beam_generate(
    ...
    *,
    ...
    queue: deque[torch.Tensor] = deque([start_ids])
    ...
    with torch.no_grad():
        input_ids = queue.popleft()
        while len(results) < 4 and rollouts < 8:
            print(f"[architect_beam] initial rollouts, progress: rollouts={rollouts}, results={len(results)}, queue={len(queue)}")
            ...
            if rollout_result is not None and rollout_result not in results:
                ...
                queue.extend(new_text_ids)
        
        while queue and rollouts < max_rollouts:
            input_ids = queue.popleft()
            rollout_result, new_text_ids = _rollout(
                model,
                tokenizer,
            ...
            if rollouts % 100 == 0:
                print(
                    f"[architect_beam] progress: rollouts={rollouts}, results={len(results)}, queue={len(queue)}"
                )
...
def _rollout(
    ...
    *,
    ...
    if cumulative < log_minprob and not do_sample:
    ...
    print(f"[architect_beam] rollout entropy stats: max={max_ent:.3f}, threshold={entropy_thr:.3f}")
    threshold = max(entropy_thr, max_ent * 0.75)  # 两个条件的并集

    # 找到第一个满足条件的位置
    for i, ent in enumerate(entropy_list):
        if ent > threshold:
            flag = True
            print(f"[architect_beam] trigger beam @len={i} entropy={ent:.3f} (max={max_ent:.3f})")
            break
    if not flag:
    ...
    # 构建新文本段列表
    new_text_ids = []
    base_seq = output.sequences[0, :prompt_len + i]
    
    for token_id in topk_token_ids:
        new_seq = torch.cat([base_seq, token_id.unsqueeze(0)])
        new_text_ids.append(new_seq)

    return (tuple(output.sequences[0].tolist()), cumulative), new_text_ids       
