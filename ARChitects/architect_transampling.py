...
from eval.core_C_hittops import GridCodec
from eval.models import HuggingFaceTextGenerator


from pathlib import Path
LOG_PATH = Path("outputs/architect_transat1.log")
    ...
    *,
    ...
    for t_idx, record in enumerate(transforms, 1):
        ...
        output = model.generate(
            input_ids=tokenizer(prompt_t, return_tensors="pt").input_ids.to(device),
            # attention_mask=torch.ones_like(input_ids, device=device),
        ...
        new_text_ids = generated_ids[len(input_ids[0]) :]
