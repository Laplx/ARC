    ...
    *,
    ...
    with torch.no_grad():
        ...
        while len(results) < 4 and rollouts < 8:
            ...
            rollout_result = tuple(output.sequences[0].tolist())
            rollouts += 1
        ...
        for result in results:
            candidates.append((tokenizer.decode(result[len(start_ids):], skip_special_tokens=False), result[1]))
...
def _rollout(
    ...
    *,
    ...
    cumulative = _logprob_given(
        model,
        tokenizer,
        tokenizer.decode(partial_generated_ids[0], skip_special_tokens=False),
        tokenizer.decode(output.sequences[0][prompt_len+prompt_orig_len:], skip_special_tokens=False),
        allowed_ids=allowed_ids,
    )
